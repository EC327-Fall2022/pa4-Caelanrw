...
#include <string> //for Trainer names
#include <iostream>
...
:GameObject(in_code),speed(5), num_wp_following(0)
{
    state = STOPPED;
...
: name(in_name), speed(in_speed), GameObject(in_code), num_wp_following(0)
{
    id_num = in_id;
    location = in_loc;
    state = STOPPED;
...
void Trainer :: ShowStatus()
{
    ...
    switch(state)
    {
        case STOPPED : cout << " stopped" << endl;
            break;
        case MOVING : cout << "moving at a speed of " << this->speed << " to destination " << this->destination << " at each step of " << this->delta;
            break;
        case MOVING_TO_GYM: cout << "heading to PokemonGym " << current_gym->GetId() << " located at " << this->destination << " at a speed of " << this->speed << " at each step of " << this->delta;
            break;
        case MOVING_TO_CENTER: cout << "heading to PokemonCenter " << current_center->GetId() << " at a speed of " << this->speed  << " at each step of " << this->delta;
            break;
        case IN_GYM: cout << "inside PokemonGym " << current_gym->GetId();
            break;
        case AT_CENTER: cout << "inside PokemonCenter " << current_center->GetId();
            break;
        case BATTLING_IN_GYM: cout << "battling in PokemonGym " << current_gym->GetId(); 
            break;
        case RECOVERING_HEALTH: cout << "recovering health in Pokemon Center " << current_center->GetId();
            break;
        case FAINTED: cout << "This trainer's Pokemon have fainted" << endl;
            break;
    }

    cout<<"Being followed by "<<num_wp_following<<" Wild Pokemon"<<endl<<endl;
